...
class SellerCLI:
    ...
    def handle_register_item(self):
        ...
        try:
            ...
            # store keywords with max 8 chars and lowercase
            keywords = [
                k.strip()[:8].lower() for k in keywords_input.split(",") if k.strip()
            ]
            if len(keywords) > 5:
    ...
    def handle_change_price(self):
        ...
        try:
            ...
            response = self.api_client.change_item_price(
                self.session, item_id, new_price
            )
    ...
    def handle_update_units(self):
        ...
        try:
            ...
            response = self.api_client.update_units_for_sale(
                self.session, item_id, quantity_change
            )

"""
Authentication utilities for seller server
"""
...
def require_auth(customer_db_pool):
    """
    Decorator factory to validate seller session for protected endpoints.

    Args:
        customer_db_pool: PostgreSQL connection pool for customer database

    Returns:
        Decorator function that validates session and injects session_id and seller_id
    """
    def decorator(f):
        @wraps(f)
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                cursor.execute(
                    "SELECT seller_id FROM seller_sessions WHERE session_id = %s "
                ...
                # Inject session_id and seller_id into the route function
                return f(session_id=session_id, seller_id=seller_id, *args, **kwargs)
